...
<x-sidebar activepage="new_combinations" :activeuser="$activeuser" />
    ...
    <div class="col-7 admin ps-4">
        <div class="row mb-2">
          <div class="col-3 text-center">
            <a href="{{ url('/new_combinations') }}" class="d-flex justify-content-center h-100 {{ isset($combinations)? 'bg-dark':'bg-danger' }} text-light p-2 text-decoration-none">Remove Filter</a>
          </div>
          <div class="col-1"></div>
          <form action="{{ route('admin.new_filterReserved') }}" method="post" class="col-4">
            @csrf
          ...
          <form action="{{ route('admin.new_filterConsecutive') }}" method="post" class="col-4">
            @csrf
          ...
          <div class="col-4"></div>
          <form action="{{ route('admin.new_filterConsecutive') }}" method="post" class="col-4 mt-2">
            @csrf
        ...
        <form action="{{ route('admin.new_filterConsecutive') }}" method="post" class="col-4 mt-2">
            @csrf
            <button type="submit" class="col-12 {{ isset($filterConsecutive5)? 'bg-success':'bg-dark' }} text-light p-2" name="filterConsecutive" value="6" style="border-left:1px solid #fff">Remove 6 Consecutives</button>
        </form>
        <div class="col-4"></div>
          <form action="{{ route('admin.new_filterConsecutive') }}" method="post" class="col-4 mt-2">
            @csrf
